...
/**
 * Заметка: здесь не отслеживается атрибут id
 */
...
export const VALID_ATTRIBUTE_NAME_REGEX = RegExp(
  '^[' + ATTRIBUTE_NAME_START_CHAR + '][' + ATTRIBUTE_NAME_CHAR + ']*$'
)

const illegalAttributeNameCache: Record<string, any> = {}
const validatedAttributeNameCache: Record<string, any> = {}
...
/**
 * Получаем свойства атрибута по HTML-имени атрибута
 */
export function getPropertyInfoByAttributeName(
  name: string
): PropertyInfo | null {
  return propertiesByAttributeName[name] || null
}
...
/**
 * Инвертированный объект свойств по имени HTML-атрибута.
 * Используется при парсинге HTML
 */
const propertiesByAttributeName: Record<string, PropertyInfo> = {}

Object.values(properties).forEach((n) => {
  propertiesByAttributeName[n.attributeName] = n
})
