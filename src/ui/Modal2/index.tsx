  ...
  return useMemo(() => {
    ...
    const modalWindow = (
      <Modal2Styled role={role} ref={ref} style={style} {...other}>
        {toolbar.length ? (
          <Modal2TitleStyled style={collapseController.openContentStyle}>
            {toolbar}
          </Modal2TitleStyled>
        ) : null}
        <Modal2ContentScrollerStyled
          style={collapseController.openContentStyle}
        >
          <Modal2ContentStyled>{children}</Modal2ContentStyled>
        </Modal2ContentScrollerStyled>
        {other.collapsedElement && (
          <div style={collapseController.collapsedContentStyle}>
            {other.collapsedElement(collapseController.toggleCollapse)}
          </div>
        )}
      </Modal2Styled>
    )
    ...
    const anchor = (
      <Modal2Styled
        ref={setRefAnchor}
        style={{ ...styleAnchor, opacity: 0, zIndex: 0 }}
        {...other}
      />
    )
...
function useCollapseModal(
  modal: HTMLDivElement | null,
  modalPosition: {
    left: number
    top: number
  } | null
): UseCollapseModalReturn {
  ...
  const style: CSSStyleDeclaration =
    modal == null ? ({} as CSSStyleDeclaration) : window.getComputedStyle(modal)
  const isCollapsed =
    collapseType === 'Collapsed' || collapseType === 'Collapsing'

  const opennedPosition: React.CSSProperties =
    currAnchor === 'Top'
      ? {
  ...
  const collapsedPosition: React.CSSProperties =
    currAnchor === 'Top'
      ? {
